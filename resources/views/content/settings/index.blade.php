...
@section('content')
    <form class="form-horizontal" onsubmit="event.preventDefault()" action="#" enctype="multipart/form-data" id="form">
      ...
      <div class="row">
        ...
        <!-- Notification & Region Settings -->
<h4 class="fw-bold py-3 mb-3">
  <span class="text-muted fw-light"></span> {{ __('Notification & Region Settings') }}
</h4>
<div class="row">
  ...
  <div class="col-md-8">
    <div class="card mb-4">
      <div class="card-body">
        <label class="form-label" for="default_region">{{ __('Default Region') }}</label>
        <select class="form-select mb-2" id="default_region" name="default_region">
            <option value="0">{{ __('No default region') }}</option>
            @foreach ($regions as $key => $value)
              <option value="{{ $key }}" @if (($settings['default_region'] ?? '') == $key) selected @endif>{{ $value }}</option>
            @endforeach
        </select>
      </div>
    </div>
        ...
        <div class="mb-3">
          ...
          <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="notifications_enabled" name="notifications_enabled"
              value="1" @if ($settings['notifications_enabled'] ?? false) checked @endif>
            <label class="form-check-label" for="notifications_enabled">{{ __('Enable Notifications') }}</label>
          </div>
        </div>
        <div class="mb-3">
          <input type="hidden" name="coupons_enabled" value="0">
          <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="coupons_enabled" name="coupons_enabled"
              value="1" @if ($settings['coupons_enabled'] ?? false) checked @endif>
            <label class="form-check-label" for="coupons_enabled">{{ __('Enable Coupons') }}</label>
          </div>
        </div>
      </div>
    </div>
