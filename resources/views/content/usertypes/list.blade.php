...
@section('title', __('Usertypes'))

@section('content')

    <h4 class="fw-bold py-3 mb-3 row justify-content-between">
        <div class="col-md-auto">
            <span class="text-muted fw-light">{{ __('Usertypes') }} /</span> {{ __('Browse User Types') }}
        </div>
        <div class="col-md-auto">
            <button type="button" class="btn btn-primary" id="create">{{ __('Add User Type') }}</button>
        </div>
    </h4>

    <!-- Basic Bootstrap Table -->
    <div class="card">
        <h5 class="card-header">{{ __('User Types Table') }}</h5>
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="fw-bold py-1 mb-1">{{ __('Add User Type') }}</h4>
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" onsubmit="event.preventDefault()" action="#"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea type="text" class="form-control" id="description_ar" name="description_ar" ></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="description_en">{{ __('Description En') }}</label>
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 1) {
                            ...
                            document.getElementById('description_ar').value = response.data.description_ar;
                            document.getElementById('description_en').value = response.data.description_en;
                            $("#modal").modal("show");
                        }
                    }
                });
            ...
            $('#submit').on('click', function() {
                ...
                if (formtype == "update") {
                    url = "{{ url('user-type/update') }}";
                    formdata.append("user_type_id", document.getElementById('id').value)
                }
